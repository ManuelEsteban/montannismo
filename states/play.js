var playState = {
    ...
    create: function() {
        ...
        this.puntuacionLabel = game.add.text(this.coinIcon.x + 32, this.coinIcon.y + 3, ' 0',{ font: '18px Arial', fill: '#ffffff' });
        ...
        /* SONIDOS */
        this.musica = game.add.audio('prueba');
        this.musica.volume = 0.4;
        this.musica.loopFull(0.4);
        this.musica.play();
    ...
    */
    updateGas: function(){
        if(this.actual_gas.width <= 0){
            ...
            this.game.add.tween(this.actual_gas).to({ width: (this.actual_gas.width - (this.total_gas / 100)) }, 200, Phaser.Easing.Linear.None, true);
        }
        ...
        if ((this.actual_gas.width >= (this.total_gas / 4) - 5) && (this.actual_gas.width <= (this.total_gas / 4) + 5) ){
            ...
            fuelLow.alpha = 0;
            game.add.tween(fuelLow).to({alpha: 0.6}, 1000).to({ alpha: 0}, 1000).start();
        }
    },
    ...
    gameOk: function() {
        ...
        youWin.alpha = 0;
        game.add.tween(youWin).to({alpha: 0.6}, 1000).to({ alpha: 0}, 1000).start();
        game.global.updateMoney = true;
        this.carSound.stop();
        this.musica.stop();
        game.time.events.add(1000, this.startMenu, this); // Despues de 1 segundo pasa a la pantalla de menu
